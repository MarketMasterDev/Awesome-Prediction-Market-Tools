...
- [MiroShark](https://github.com/aaronjmars/MiroShark) — Open-source swarm-intelligence engine that simulates a Polymarket-style prediction market alongside Twitter and Reddit: hundreds of grounded LLM personas trade and post hour-by-hour, surfacing how a scenario moves probabilities. Counterfactual branching and per-agent MCP tools; ~$1 per run.
- [Aeon](https://github.com/aaronjmars/aeon) — Autonomous agent framework (runs unattended on GitHub Actions) with built-in skills that monitor Polymarket and Kalshi on a schedule and alert on probability shifts, narrative moves, and arbitrage. MIT.
